...
let admin = {
    email: 'haidang02032003@gmail.com',
    password: '123456',
    fullname: 'Admin',
    orders: [
        {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    Validator({
        ...
        onSubmit: function (data) {
            ...
            if (getAcount) {
                ...
                if (nick.email == 'haidang02032003@gmail.com') {
                    checkAdmin = true;
                } else {
                    checkAdmin = false;
                }
    ...
    Validator({
        ...
        onSubmit: function (data) {
            ...
            window.localStorage.setItem('haidang02032003@gmail.com', JSON.stringify(admin));
            customerOrder = {};
            customerOrder.code = randomCodeOrders();
            customerOrder.infor = {};
    ...
    Validator({
        ...
        onSubmit: function (data) {
            ...
            if (!checkNameProduct) handleErrLogin('#form-4', 0, 'Nhập sai tên sản phẩm');
...
function render_product(index, wrapper_product, infor_product) {
    if (wrapper_product.classList.length < 2) wrapper_product.classList.add(index);
    ...
    for (var i = 0; i < quantity_loop; i++) {
        ...
        product_add.innerHTML = `
          ...
          <span class="product-quantities">SL: <span>${infor_product[index].quantity[i]}</span></span>
            `;
        ...
        var infor_product = [
            {
                ...
                price: ['190000₫', '250000₫', '534000₫', '260000₫', '251000₫', '356000₫', '220000₫', '340000₫'],
            ...
            },
            {
                ...
                price: ['289000₫', '399000₫', '199000₫', '280000₫', '380000₫', '400000₫', '599000₫', '320000₫'],
            ...
            },
            {
                ...
                price: ['270000₫', '380000₫', '499000₫', '280000₫', '350000₫', '199000₫', '270000₫', '330000₫'],
        ...
        check_product.forEach((item) => {
            if (name_product.innerText == item.querySelector('p').innerText) {
                if (item.querySelector('.quantities').value * 1 < currentQuantity) {
                    item.querySelector('.quantities').value = item.querySelector('.quantities').value * 1 + 1;
                }
                flag = 0;
            }
        });
...
function reRenderQuantity(list_buy) {
    ...
    list_buy.forEach((element, index) => {
        list_product_home.forEach((item) => {
            if (element.querySelector('p').innerText == item.querySelector('p').innerText) {
                var setQuantity = item.querySelector('.product-quantities span').textContent;
                item.querySelector('.product-quantities span').textContent =
                    Number(setQuantity) - Number(element.querySelector('.quantities').value);
            }
        });
    });
}
...
buy.onclick = function () {
    if (checkUser) {
        ...
        if (list_buy.length < 1) showErrorToast('Giỏ hàng của bạn chưa có sản phẩm');
        ...
        $('#form-3 .form-submit').onclick = () => {
            ...
            for (var i = 0; i < list_buy.length; i++) {
                ...
                inforOrder.price = list_buy[i].querySelector('.your-product-price').textContent;
                    ...
                    for (const [key, values] of Object.entries(product)) {
                        ...
                        if (key == 'name') {
                            ...
                            product[key][indexPrice] =
                                product[key][indexPrice] - list_buy[i].querySelector('.quantities').value;
                        }
                    }
                });
          ...
          `;
            ...
            icon_check.forEach((item) => {
                item.addEventListener('click', (e) => {
                    ...
                    let s_code = code_product.slice(code_product.length - 7, code_product.length);
...
function getPrice(index, start, end) {
    ...
    for (let i = 0; i < admin.orders.length; i++) {
        ...
        if (index == 0 || index == 1) {
            day = index == 0 ? admin.orders[i].date : admin.orders[i].date.slice(start, end);
        } else {
        ...
        for (let j = i + 1; j < admin.orders.length; j++) {
            let day_next;
            if (index == 0 || index == 1) {
                day_next = index == 0 ? admin.orders[j].date : admin.orders[j].date.slice(start, end);
            } else {
